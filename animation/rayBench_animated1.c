...
#pragma acc declare create(myScene)

struct color bgColor;
struct point camera;

struct color canvas[1920][1080];
...
struct point CanvasToViewport(int x, int y){
	struct point i;
	i.x = (float)x * 1.0 / (float)1920.0; i.y = (float)y * (float)0.6 / (float)1080.0; i.z = (float)1.0;
...
void save(struct color canvas[screenWidth][screenHeight], int id){ //pass in array in C
	char filename[50];
